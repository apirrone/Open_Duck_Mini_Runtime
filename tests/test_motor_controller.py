...
def main():
    ...
    for joint_name, joint_id in hwi.joints.items():
        try:
            ...
            print(
                f"✓ Low torque set successfully for motor '{joint_name}' (ID: {joint_id})."
            )
        except Exception as e:
            print(
                f"✗ Error setting low torque for motor '{joint_name}' (ID: {joint_id}): {e}"
            )
            print(f"Error details: {traceback.format_exc()}")
    ...
    for joint_name, joint_id in hwi.joints.items():
        ...
        if (joint_name, joint_id) in unresponsive_motors:
            print(
                f"Skipping previously unresponsive motor: '{joint_name}' (ID: {joint_id})"
            )
            continue

        print(
            f"Attempting to read position from motor '{joint_name}' (ID: {joint_id})..."
        )
        try:
            # Try to read the position to check if motor is responsive
            position = hwi.io.read_present_position([joint_id])
            print(
                f"✓ Motor '{joint_name}' (ID: {joint_id}) is responsive. Position: {position[0]:.3f}"
            )
        except Exception as e:
                ...
                for joint_name, joint_id in hwi.joints.items():
                    if (joint_name, joint_id) not in unresponsive_motors:
                        try:
                            ...
                            print(
                                f"Disabled torque for motor '{joint_name}' (ID: {joint_id})"
                            )
                        except:
                            pass
            except:
    ...
    for joint_name, joint_id in hwi.joints.items():
        ...
        test_this_motor = input(
            f"Test this motor? (Enter/y for yes, n to skip, q to quit): "
        ).lower()
        ...
        try:
            ...
            print(
                f"Reading current position from motor '{joint_name}' (ID: {joint_id})..."
            )
            ...
            print(
                f"Moving motor '{joint_name}' (ID: {joint_id}) to test position: {test_position:.3f}..."
            )
            ...
            print(
                f"Returning motor '{joint_name}' (ID: {joint_id}) to original position..."
            )
    ...
    for joint_name, joint_id in hwi.joints.items():
        if (joint_name, joint_id) in unresponsive_motors:
            print(
                f"Skipping turning off unresponsive motor: '{joint_name}' (ID: {joint_id})"
            )
            continue
        ...
        try:
            ...
            for joint_name, joint_id in hwi.joints.items():
                try:
                    ...
                    hwi.io.disable_torque([joint_id])
                    print(
                        f"✓ Motor '{joint_name}' (ID: {joint_id}) turned off successfully."
                    )
                except Exception as e:
                    print(
                        f"✗ Error turning off motor '{joint_name}' (ID: {joint_id}): {e}"
                    )
        except Exception as e:
