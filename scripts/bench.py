...
for i in range(500):
    ...
    s = time.time()
    dof_pos2, dof_vel2 = hwi.get_present_positions_and_velocities()
    took = time.time() - s
    times["get_pos_vel_all"].append(took)
    # print(np.around(np.array(dof_pos) - np.array(dof_pos2), 3))
    # print(np.around(np.array(dof_vel) - np.array(dof_vel2), 3))
    # print("==")
...
report["get_pos_vel_all_mean"] = np.mean(times["get_pos_vel_all"])
report["get_pos_vel_all_std"] = np.std(times["get_pos_vel_all"])
