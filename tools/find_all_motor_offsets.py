...
def main():
    ...
    input(
        "Press Enter to start. At any time, press Ctrl+C to stop and turn off motors."
    )
    ...
    for joint_name, joint_id in hwi.joints.items():
        try:
            ...
            if current_positions[joint_name] is not None:
                ...
                print(
                    f"Cannot calculate offset for '{joint_name}' (ID: {joint_id}) - initial position unknown"
                )
                offsets[joint_name] = 0
        except Exception as e:
            print(
                f"Error reading new position for '{joint_name}' (ID: {joint_id}): {e}"
            )
            offsets[joint_name] = 0
    ...
    if confirm == "y":
        ...
        update = input(
            "\nWould you like to update ~/duck_config.json with these offsets? (y/N): "
        ).lower()
        if update == "y":
            import os, json
            ...
            try:
                ...
                config["joints_offsets"] = {
                    k: float(f"{v:.6f}") for k, v in offsets.items()
                }
